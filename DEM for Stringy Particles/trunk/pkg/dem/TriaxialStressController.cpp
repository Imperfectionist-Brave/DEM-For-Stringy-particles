...
void TriaxialStressController::updateStiffness()//更新刚度
{
			...
			if (fn != 0) {
				...
				for (int index = 0; index < 6; ++index)
					if (wall_id[index] == id1 || wall_id[index] == id2) {
						...
						stiffness[index] += currentContactPhysics->kn;//计算六面总刚度
					}
			}
		}
	if (fluidStiffness > 0) {//    无用
...
{
	scene->forces.sync();
	Real       translation = normal[wall].dot(getForce(scene, wall_id[wall]) - resultantForce);//当前力-目标力 力差值
	...
	if (translation != 0) {
		if (stiffness[wall] != 0) {
			translation /= stiffness[wall];//需求位移增量
			if (log) TRVAR2(translation, wall_max_vel * scene->dt)
			translation = math::min(math::abs(translation), wall_max_vel * scene->dt) * math::sign(translation);//取较小位移增量
		} else//接触总刚度为0
	...
	p->vel = previousTranslation[wall] / scene->dt;//给定速度在此步中施加位移
...
void TriaxialStressController::action()
{
	...
	height          = p_top->se3.position.y() - p_bottom->se3.position.y() - thickness;//实时尺度计算
	width           = p_right->se3.position.x() - p_left->se3.position.x() - thickness;
	depth           = p_front->se3.position.z() - p_back->se3.position.z() - thickness;

	boxVolume = height * width * depth;         //盒子体积 随加载变化
	if ((first) || (updatePorosity)) {
		...
		for (const auto& b : *scene->bodies) {
			if (b->isClump()) {
				...
				particlesVolume += 1.3333333 * Mathr::PI * pow(sphere->radius, 3);   //计算粒子总体积
			}
		}
		first          = false;
		updatePorosity = false;
	}
	max_vel1 = 3 * width / (height + width + depth) * max_vel;//max_vel=1 x方向最大速度
	max_vel2 = 3 * height / (height + width + depth) * max_vel;//max_vel=1 y方向最大速度
	max_vel3 = 3 * depth / (height + width + depth) * max_vel;//max_vel=1 z方向最大速度

	porosity        = (boxVolume - particlesVolume) / boxVolume;//孔隙率
	...
	if (scene->iter % stiffnessUpdateInterval == 0 || scene->iter < 100) updateStiffness();//更新刚度 每n次迭代更新
	bool isARadiusControlIteration = (scene->iter % radiusControlInterval == 0);//

	if (scene->iter % computeStressStrainInterval == 0 || (internalCompaction && isARadiusControlIteration)) computeStressStrain();//计算应力应变

	if (!internalCompaction) {//非颗粒长大固结
		Vector3r wallForce(0, goal2 * width * depth, 0);//y方向目标作用力
		if (wall_bottom_activated) {
			if (stressMask & 2) controlExternalStress(wall_bottom, wallForce, p_bottom, max_vel2);//墙面应力伺服控制
			else {//采用应变控制？
	...
	} else //if internal compaction  颗粒长大压实
	{
		p_bottom->vel = Vector3r::Zero();
		p_top->vel    = Vector3r::Zero();
		p_left->vel   = Vector3r::Zero();
...
{
	scene->forces.sync();
	State* p_bottom = Body::byId(wall_bottom_id, scene)->state.get();//获取state
	State* p_top    = Body::byId(wall_top_id, scene)->state.get();
	State* p_left   = Body::byId(wall_left_id, scene)->state.get();
	State* p_right  = Body::byId(wall_right_id, scene)->state.get();
	State* p_front  = Body::byId(wall_front_id, scene)->state.get();
	...
	height = p_top->se3.position.y() - p_bottom->se3.position.y() - thickness;//计算盒子尺寸
	width  = p_right->se3.position.x() - p_left->se3.position.x() - thickness;
	depth  = p_front->se3.position.z() - p_back->se3.position.z() - thickness;
	...
	strain[0]        = log(width / width0); // all strain values are positiv for extension 轴向应变
	strain[1]        = log(height / height0);
	strain[2]        = log(depth / depth0);
	volumetricStrain = strain[0] + strain[1] + strain[2];

	Real invXSurface = 1.f / (height * depth);//1/x方向墙面积
	Real invYSurface = 1.f / (width * depth);//1/y方向墙面积
	Real invZSurface = 1.f / (width * height);//1/z方向墙面积

	force[wall_bottom]  = getForce(scene, wall_id[wall_bottom]);//计算各方向应力
...
void TriaxialStressController::controlInternalStress(Real multiplier)//控制颗粒长大函数
{
	particlesVolume *= pow(multiplier, 3);
	Shop::growParticles(multiplier, true, true);//增长各个颗粒
}
