...
#include"Nodedriver.hpp"
#include<core/Scene.hpp>
#include<core/Clump.hpp>
#include<pkg/dem/LineSegment.hpp>
//#include<sudodem/lib/base/Math.hpp>

namespace yade { // Cannot have #include directive inside.
	YADE_PLUGIN((NODEdriver));
	CREATE_LOGGER(NODEdriver);

	// 1st order numerical damping
	void NODEdriver::cundallDamp1st(Vector3r& force, const Vector3r& vel) {
		for (int i = 0; i < 3; i++) force[i] *= 1 - damping * math::sign(force[i] * vel[i]);
	}
	// 2nd order numerical damping
	void NODEdriver::cundallDamp2nd(const Real& dt, const Vector3r& vel, Vector3r& accel) {
		for (int i = 0; i < 3; i++) accel[i] *= 1 - damping * math::sign(accel[i] * (vel[i] + 0.5*dt*accel[i]));
	}

	Vector3r NODEdriver::computeAccel(const Vector3r& force, const Real& mass, int blockedDOFs) {//计算加速度
	...
	/*Vector3r NODEdriver::computeAngAccel(const Vector3r& torque, const Vector3r& inertia, int blockedDOFs) {
...
#ifdef YADE_OPENMP
	void NODEdriver::ensureSync()
	{
		if (syncEnsured) return;
		YADE_PARALLEL_FOREACH_NODE_BEGIN(const shared_ptr<Node>& b, scene->_nodes) {
			// 		if(b->isClump()) continue;
			scene->nodeforces.addForce(b->getId(), Vector3r(0, 0, 0));
		} YADE_PARALLEL_FOREACH_NODE_END();
		syncEnsured = true;
	}
#endif


//	void NODEdriver::applyNodalForces() {//根据三角单元施加对节点的力
//		Real dt = scene->dt;
//		scene->forces.sync();
...
//	}
	...
	void NODEdriver::applyNodalForces() {//根据Segment单元施加对节点的力 
			[[maybe_unused]]Real dt = scene->dt;
			scene->forces.sync();
			YADE_PARALLEL_FOREACH_BODY_BEGIN(const shared_ptr<Body>& b, scene->bodies) {
				// clump members are handled inside clumps
				//if(b->isClumpMember()) continue;
				if (b->shape->getClassName() != "Segment") continue;
				[[maybe_unused]]const Body::id_t& id = b->getId();
				const shared_ptr<Segment>& se = YADE_PTR_CAST<Segment>(b->shape);
				//if(se==NULL)continue;
				//b->state->pos = Elm->pos;
				//b->state->ori = Elm->ori;
				//if(!particle->contacts.empty()) distributeForces(particle,sh->cast<Facet>(),bending||applyBary);
				...
				//Vector3r fc = Vector3r::Zero();
				Vector3r fin1 = Vector3r::Zero();
				Vector3r fin2 = Vector3r::Zero();
				//Vector3r mc = Vector3r::Zero();
				//fc = scene->forces.getForce(id); //mc = scene->forces.getTorque(id);
				Real Deformation = se->getLength() - se->length0;
				Real Fn = 0;
				if (Deformation <= 0);
				else {//弹性内力
					Fn = kn * Deformation;
					//cout<<"Fn="<<Fn<<endl;
					//se->node1->getId()
					Vector3r tan1 = se->gettan1();
					Vector3r tan2 = -se->gettan1();
					fin1 = Fn * tan1;
					fin2 = Fn * tan2;
					//cout<<"tan1="<<tan1<<"tan2="<<tan2<<endl;
					//cout<<"Fn="<<Fn<<"Fin1="<<fin1<<"Fin2="<<fin2<<endl;
				}
				if(!se->middle){se->isFirst? se->addForce(scene,se->id,fin1):se->addForce(scene,se->id,fin2);
					se->updatePos();
				}
				scene->nodeforces.addForce(se->node1->getId(), fin1); 
				scene->nodeforces.addForce(se->node2->getId(), fin2);
				se->updateBodyPos();
				//update node pos
				
		} YADE_PARALLEL_FOREACH_BODY_END();
	}
	void NODEdriver::applySphereForces() {//根据Segment单元 施加对颗粒的力
			[[maybe_unused]]Real dt = scene->dt;
			scene->forces.sync();
			YADE_PARALLEL_FOREACH_BODY_BEGIN(const shared_ptr<Body>& b, scene->bodies) {
				// clump members are handled inside clumps
				...
				//const Body::id_t& id = b->getId();
				const shared_ptr<Segment>& se = YADE_PTR_CAST<Segment>(b->shape);
				//se->updatePos();//update pos
				if(se->middle) continue;
				//Vector3r fc = Vector3r::Zero();
				Vector3r fin1 = Vector3r::Zero();
				Vector3r fin2 = Vector3r::Zero();
				Real Deformation = se->getLength() - se->length0;
				...
				if(se->isFirst)/*se->applyForce(scene,fin1);*/se->addForce(scene,se->id,fin1);
				else /*se->applyForce(scene,fin2);*/se->addForce(scene,se->id,fin2);
				//if (!se->middle){
					//se->updatePos();
					//if(se->isFirst)/*se->applyForce(scene,fin1);*/se->addForce(scene,se->id,fin1);
					//else /*se->applyForce(scene,fin2);*/se->addForce(scene,se->id,fin2);
				//}
				se->updatePos();//update conneected with sphere node's pos
				//se->state->pos=se->node1->pos;
		} YADE_PARALLEL_FOREACH_BODY_END();
	}
	void NODEdriver::driveNodes() {
		Real dt = scene->dt;
		YADE_PARALLEL_FOREACH_NODE_BEGIN(const shared_ptr<Node>& b, scene->_nodes) {
...
#ifdef YADE_DEBUG
			if (isnan(f[0]) || isnan(f[1]) || isnan(f[2])) throw runtime_error(("NODEdriver: NaN force acting on #" + boost::lexical_cast<string>(id) + ".").c_str());
			if (isnan(m[0]) || isnan(m[1]) || isnan(m[2])) throw runtime_error(("NODEdriver: NaN torque acting on #" + boost::lexical_cast<string>(id) + ".").c_str());
			if (b->mass <= 0 && ((b->blockedDOFs & Node::DOF_XYZ) != Node::DOF_XYZ)) throw runtime_error(("NODEdriver: #" + boost::lexical_cast<string>(id) + " has some linear accelerations enabled, but State::mass is non-positive."));
			if (b->inertia.minCoeff() <= 0 && ((b->blockedDOFs & Node::DOF_RXRYRZ) != Node::DOF_RXRYRZ)) throw runtime_error(("NODEdriver: #" + boost::lexical_cast<string>(id) + " has some angular accelerations enabled, but inertia contains non-positive terms."));
#endif
	...
	/*Quaternionr NODEdriver::DotQ(const Vector3r& angVel, const Quaternionr& Q) {
		Quaternionr dotQ;
