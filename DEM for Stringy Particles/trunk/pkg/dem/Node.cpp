...
namespace yade { // Cannot have #include directive inside.
	...
	const shared_ptr<Node>& Node::byId(Node::id_t _id, Scene* rb) { return (*((rb ? rb : Omega::instance().getScene().get())->_nodes))[_id]; }
	const shared_ptr<Node>& Node::byId(Node::id_t _id, shared_ptr<Scene> rb) { return (*(rb->_nodes))[_id]; }
...
#undef _GET_DOF
	...
	bool NodeContainer::erase(Node::id_t id) {//default is false (as before)
		...
		[[maybe_unused]]const shared_ptr<Node>& b = Node::byId(id);
		node[id].reset();
		return true;
	}
}
