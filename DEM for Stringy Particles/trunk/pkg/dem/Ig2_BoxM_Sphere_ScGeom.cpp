...
#include "Ig2_BoxM_Sphere_ScGeom.hpp"
...
YADE_PLUGIN((Ig2_BoxM_MembraneNode_ScGeom)(Ig2_BoxM_Sphere_ScGeom)(BoxM)(Bo1_BoxM_Aabb)(Gl1_BoxM));
bool Ig2_BoxM_MembraneNode_ScGeom::go(
        ...
        const shared_ptr<Interaction>& c)
{
	...
	BoxM*    obb = static_cast<BoxM*>(cm1.get());
	MembraneNode* s   = static_cast<MembraneNode*>(cm2.get());
...
bool Ig2_BoxM_Sphere_ScGeom::go(
        const shared_ptr<Shape>&       cm1,
        const shared_ptr<Shape>&       cm2,
        const State&                   state1,
        const State&                   state2,
...
bool Ig2_BoxM_MembraneNode_ScGeom::goReverse(
        const shared_ptr<Shape>&       cm1,
        const shared_ptr<Shape>&       cm2,
        const State&                   state1,
        const State&                   state2,
...
bool Ig2_BoxM_Sphere_ScGeom::goReverse(
        const shared_ptr<Shape>&       cm1,
        const shared_ptr<Shape>&       cm2,
        const State&                   state1,
        const State&                   state2,
...
void Bo1_BoxM_Aabb::go(const shared_ptr<Shape>& cm, shared_ptr<Bound>& bv, const Se3r& se3, const Body* /*b*/)
{
	BoxM* box = static_cast<BoxM*>(cm.get());
...
#ifdef YADE_OPENGL
void Gl1_BoxM::go(const shared_ptr<Shape>& cg, const shared_ptr<State>&, bool wire, const GLViewInfo&)
{
	glColor3v(cg->color);
	Vector3r& extents = (static_cast<BoxM*>(cg.get()))->extents;
