...
bool Law2_ScGeom_WirePhys_WirePM::go(shared_ptr<IGeom>& ig, shared_ptr<IPhys>& ip, Interaction* contact)
{
	...
	Real displN = geom->penetrationDepth; // NOTE: ScGeom -> penetrationDepth>0 when spheres interpenetrate, and therefore, for wire always negative线拉伸时时小于零
	...
	Real D = displN - phys->initD; // interparticular distance is computed depending on the equilibrium distance  计算力的线距离
	...
	if (!scene->isPeriodic) applyForceAtContactPoint(f, geom->contactPoint, id2, st2->se3.position, id1, st1->se3.position);//position用于计算矢径
...
void Ip2_WireMat_WireMat_WirePhys::go(const shared_ptr<Material>& b1, const shared_ptr<Material>& b2, const shared_ptr<Interaction>& interaction)
{
	...
	kValues.push_back(k);
	for (unsigned int i = 1; i < DFValues.size(); i++) {
		Real deltau = -DFValues[i](0) + DFValues[i - 1](0);//>0
		Real deltaF = -DFValues[i](1) + DFValues[i - 1](1);//>0
		k           = deltaF / deltau;//k>0
		kValues.push_back(k);
	}
