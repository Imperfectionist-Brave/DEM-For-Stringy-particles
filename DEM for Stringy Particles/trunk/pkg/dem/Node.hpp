...
namespace yade { // Cannot have #include directive inside.
	...
	public:
		...
		YADE_CLASS_BASE_DOC_ATTRS_CTOR_PY(Node, Serializable, "A node for fem elements; no interaction with other Nodes.",
...
#if YADE_OPENMP
#define YADE_PARALLEL_FOREACH_NODE_BEGIN(b_,nodes) const Node::id_t _sz(nodes->size()); _Pragma("omp parallel for") for(Node::id_t _id=0; _id<_sz; _id++){ if(!(*nodes)[_id])  continue; b_((*nodes)[_id]);
#define YADE_PARALLEL_FOREACH_NODE_END() }
#else
#define YADE_PARALLEL_FOREACH_NODE_BEGIN(b,nodes) FOREACH(b,*(nodes)){
#define YADE_PARALLEL_FOREACH_NODE_END() }
#endif
	...
	*/
	...
	public:
		...
		public:
			...
			smart_iterator(const smart_iterator& source):ContainerT::iterator(NULL) { (*this) = source; end = source.end; }//2024-5-29
		};
