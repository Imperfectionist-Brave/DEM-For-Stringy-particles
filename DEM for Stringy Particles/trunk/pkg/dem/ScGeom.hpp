...
\t\\Delta\\vec{x}_{12}^s=\\Delta t \\vec{v}_{12}^s.",
		...
		//((Real,penetrationDepth1,NaN,(Attr::noSave|Attr::readonly),"Penetration distance of spheres (positive if overlapping)"))//线间隔距离***2023-12-2
		((Vector3r,shearInc,Vector3r::Zero(),(Attr::noSave|Attr::readonly),"Shear displacement increment in the last step"))
		//((Vector3r,startAcPoint1,Vector3r::Zero(),,"Force application point 1"))//scgeom起始点1（球为中心的局部坐标系）***
		//((Vector3r,startAcPoint2,Vector3r::Zero(),,"Force application point 2"))//scgeom起始点2（球为中心的局部坐标系）***
		//((Vector3r,currentAcPoint1,Vector3r::Zero(),,"Force application point 1"))//scgeom当前作用点1***
		//((Vector3r,currentAcPoint2,Vector3r::Zero(),,"Force application point 2"))//scgeom当前作用点2***
		,
