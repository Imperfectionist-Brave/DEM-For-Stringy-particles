...
//#include <pkg/common/Line.hpp>//9-9
//#include <pkg/dem/LineContactLaw.hpp>//6-13
...
void OpenGLRenderer::renderIGeom()
{
	...
	//geomDispatcher.updateScenePtr();
	{
		...
		for (const auto& I : *scene->interactions) {
			...
			if(!geom) continue;
			Vector3r relIgeom1=geom->currentAcPoint1;				
			Vector3r relIgeom2=geom->currentAcPoint2;
			glColor3v(Vector3r(1, 0, 1));
			glPushMatrix();
			glBegin(GL_LINES);
			glVertex3v(relIgeom1);
			glVertex3v(relIgeom2);//选取两点绘制一条直线
			glEnd();
			glPopMatrix();*/
			//@@@
...
void OpenGLRenderer::renderLineIPhys()
{
	physDispatcher.scene = scene.get();
	physDispatcher.updateScenePtr();
	{
...
void OpenGLRenderer::renderShape()
{
	...
	for (const auto& b : *scene->bodies) {
		...
		if(!renderSphere&&b->shape->getClassName() == "Sphere") continue;//renderMembrane过滤调sphere
		if(!renderLine&&(b->shape->getClassName() == "LineConnection"||b->shape->getClassName() == "LineNode"))continue;
		if(!renderMembrane&&b->shape->getClassName() == "MembraneNode") continue;
		if(idfilter&&b->getId()!=showid)continue;//根据id显示
...
//************9-2
void OpenGLRenderer::Threadshow([[maybe_unused]]int _model,[[maybe_unused]]Real _threshold){//9-9
	shapeDispatcher.scene = scene.get();
	shapeDispatcher.updateScenePtr();
	
	
	if(update){//根据力阈值筛选柔索段
		const shared_ptr<ThreadContainer>& TContainer = scene->threads;
		[[maybe_unused]]int Num = TContainer->size();
		assert(Num > 0);
		...
		std::vector<int> showidnode;showidnode.clear();
		std::vector<int> showidsphere;showidsphere.clear();
		std::vector<int> showidline;showidline.clear();
		std::vector<int> showidsphere_contact;showidsphere_contact.clear();
		if(_model<=0){//显示拉伸状态但无力作用柔索段
			for (ThreadContainer::iterator itt = First; itt != End; itt++) {
				const shared_ptr<Thread>& n = *itt;
				//LineNodeGeom*     geom=NULL;
				//LinePhys* phys=YADE_CAST<LinePhys*>(n->phys[0].get());
				Real diff=n->length-n->length0;
				if(diff<0&&n->get_f()<=0){
					for(std::vector<int>::iterator itn=n->nodeid.begin();itn!=n->nodeid.end();itn++){//添加节点🆔id 以及节点接触球id
						showidnode.push_back(*itn);
						const shared_ptr<Body> &b=Body::byId(*itn,scene); 
						for(std::map<Body::id_t, shared_ptr<Interaction> >::iterator itintr=b->intrs.begin();itintr!=b->intrs.end();itintr++){//添加存在接触颗粒
							if(!(*itintr).second->isReal()) continue;
							//const shared_ptr<Interaction> &intr=(*itintr).second;
							int idintr=(*itintr).first;
							auto it1 = std::find(n->lineid.begin(), n->lineid.end(), idintr);
							if (it1 !=n->lineid.end())continue;
							auto it2 = std::find(showidsphere.begin(), showidsphere.end(), idintr);
							if (it2 ==showidsphere.end())showidsphere.push_back(idintr);
						}
						
					}
					
					for(std::vector<int>::iterator itl=n->lineid.begin();itl!=n->lineid.end();itl++){//添加line id
						showidline.push_back(*itl);
					}
					auto it1 = std::find(showidsphere.begin(), showidsphere.end(), n->sphid[0]);//添加前后颗粒id
    					if (it1 == showidsphere.end())showidsphere.push_back(n->sphid[0]);
    					auto it2 = std::find(showidsphere.begin(), showidsphere.end(), n->sphid[1]);
    					if (it2 == showidsphere.end())showidsphere.push_back(n->sphid[1]);
				}
				else continue;
				
			}
			/*for(std::vector<int>::iterator it=showidsphere_contact.begin();it!=showidsphere_contact.end();it++){
				auto it1 = std::find(showidsphere.begin(), showidsphere.end(),*it);
				if (it1 == showidsphere.end())showidsphere.push_back(*it);
				else continue;
				contact_thread_num+=1;
			}*/
			
			showids.clear();
			for(std::vector<int>::iterator it=showidnode.begin();it!=showidnode.end();it++){showids.push_back(*it);}
			for(std::vector<int>::iterator it=showidsphere.begin();it!=showidsphere.end();it++){showids.push_back(*it);}
			for(std::vector<int>::iterator it=showidline.begin();it!=showidline.end();it++){showids.push_back(*it);}
			
		}
		else if(_model>0&&_threshold>0){
			for (ThreadContainer::iterator itt = First; itt != End; itt++) {
				const shared_ptr<Thread>& n = *itt;
				//Real diff=n->length-n->length0;
				if(n->get_f()>=_threshold){
					for(std::vector<int>::iterator itn=n->nodeid.begin();itn!=n->nodeid.end();itn++){
						showidnode.push_back(*itn);
						const shared_ptr<Body> &b=Body::byId(*itn,scene); 
						for(std::map<Body::id_t, shared_ptr<Interaction> >::iterator itintr=b->intrs.begin();itintr!=b->intrs.end();itintr++){//添加存在接触颗粒
							if(!(*itintr).second->isReal()) continue;
					...
					for(std::vector<int>::iterator itl=n->lineid.begin();itl!=n->lineid.end();itl++){
						showidline.push_back(*itl);
					}
					auto it1 = std::find(showidsphere.begin(), showidsphere.end(), n->sphid[0]);
    					if (it1 == showidsphere.end())showidsphere.push_back(n->sphid[0]);
    					auto it2 = std::find(showidsphere.begin(), showidsphere.end(), n->sphid[1]);
    					if (it2 == showidsphere.end())showidsphere.push_back(n->sphid[1]);
				}
	...
	const std::lock_guard<std::mutex> lock(scene->bodies->drawloopmutex);
	for (const auto& _id : showids) {
		//cout<<"id is :"<<_id<<endl;
		const shared_ptr<Body> &b=Body::byId(_id,scene);
		if (!b || !b->shape) continue;
		if(!renderSphere&&b->shape->getClassName() == "Sphere") continue;//renderMembrane过滤调sphere
		if(!renderLine&&(b->shape->getClassName() == "LineConnection"||b->shape->getClassName() == "LineNode"))continue;
		if(!renderMembrane&&b->shape->getClassName() == "MembraneNode") continue;
...
bool 	OpenGLRenderer::change_ids(const boost::python::list&  pylist)
{
		int num_bodies = boost::python::len(pylist);
		assert(num_bodies>0);
		showids.clear();
		for(int i=0;i<num_bodies;i++){
			//showids[i]=py::extract<int>(pylist[i]);
			showids.push_back(boost::python::extract<int>(pylist[i]));
		}
		return int(showids.size())==num_bodies;
}

int OpenGLRenderer::get_num_bodies()
	{
		return int(showids.size());
	}
