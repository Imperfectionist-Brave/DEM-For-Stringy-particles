...
public:
	...
	//vector<int> showids;
	void 							Threadshow(int _model,Real _threshold);//9-2
	bool 							change_ids(const py::list&  pylist);
	//bool 							change_ids(vector<int> ID);
	...
	YADE_CLASS_BASE_DOC_ATTRS_CTOR_PY(OpenGLRenderer,Serializable,"Class responsible for rendering scene on OpenGL devices.",
		...
		((bool,renderSphere,true,,"Render body :yref:`sphere`"))
		((bool,Threadshape,false,,"Render body :yref:`Shape`"))
		((bool,update,false,,"Render body :yref:`Shape`"))
		((int,model,1,,"Render body :yref:`Shape`"))//-9-8  >0 显示真实柔索拉状态
		((int,contact_thread_num,0,Attr::readonly,"Render body :yref:`Shape`"))//-9-8  >0 显示柔索与外部颗粒接触数量
		((Real,Tforce_threshold,0.1,,"Render body :yref:`Shape`"))//-9-8  设定最小柔索力阈值
		((bool,idfilter,false,,"Render body :yref:`sphere`"))
		((int,showid,-1,,"Artificially enlarge (scale) rotations of bodies relative to their :yref:`reference orientation<State.refOri>`, so the they are better visible."))
		((bool,renderLine,true,,"Render body :yref:`sphere`"))
		((bool,renderMembrane,true,,"Render body :yref:`sphere`"))
		...
		.def("change_ids",&OpenGLRenderer::change_ids,"Make body visible (see :yref:`OpenGLRenderer::hideBody`)")//(boost::python::arg("pylist")),
		.def("get_num_bodies",&OpenGLRenderer::get_num_bodies,(boost::python::arg("none")),"Make body visible (see :yref:`OpenGLRenderer::hideBody`)")//(boost::python::arg("pylist")),
	);
	// clang-format on
};
