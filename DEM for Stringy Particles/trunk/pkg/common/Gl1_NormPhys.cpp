...
//#include <pkg/common/Line.hpp>//6-12
...
Real        Gl1_NormPhys::maxFnNegative;
Real        Gl1_NormPhys::refRadius;
Real        Gl1_NormPhys::maxRadius;
bool        Gl1_NormPhys::test;//
bool        Gl1_NormPhys::showall;
bool        Gl1_NormPhys::initial;
Real        Gl1_NormPhys::ratio;
Real        Gl1_NormPhys::k;//
...
void Gl1_NormPhys::go(const shared_ptr<IPhys>& ip, const shared_ptr<Interaction>& i, const shared_ptr<Body>& b1, const shared_ptr<Body>& b2, bool /*wireFrame*/)
{
	...
	if(test)cout<<"phys name is"<<"  "<<ip->getClassName()<<endl;//6-12
	GenericSpheresContact* geom = YADE_CAST<GenericSpheresContact*>(ig.get());
	//NodeScGeom2* geom2=YADE_CAST<NodeScGeom2*>(ig.get());//6-12
	//if(geom2)return;
	...
	if(showall&&signFilter<0){
		//assert(ratio>0);
		if (ip->getClassName() != "LinePhys")return;
		LinePhys*         lp = static_cast<LinePhys*>(ip.get());
		LineNodeGeom* geom2=YADE_CAST<LineNodeGeom*>(ig.get());
		if(initial){
			if(geom2->isFirst||geom2->isLast){fnNorm = lp->_normalForce.dot(geom->normal);}
			else fnNorm =lp->normalForce.dot(geom->normal);
		}
		else {
			fnNorm = lp->Fn;
		}
	...
	if (fnSign < 0) {
			maxFnNegative=min(-fnNorm,maxFnNegative);
		        p1        = b1->state->pos;//***添加行
		        p2        = b2->state->pos;//***添加行
	}
