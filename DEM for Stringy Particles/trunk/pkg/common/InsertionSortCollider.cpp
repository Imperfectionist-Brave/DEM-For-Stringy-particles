...
void InsertionSortCollider::handleBoundInversion(Body::id_t id1, Body::id_t id2, InteractionContainer* interactions, Scene*)
{
	...
	if (spatialOverlap(id1, id2) && Collider::mayCollide(Body::byId(id1, scene).get(), Body::byId(id2, scene).get()) && !interactions->found(id1, id2))
		...
		/*std::cout<<"creat interaction of "<<id1<<"and"<<id2<<std::endl;
		//**********
		std::cout<<id1<<"minx"<<minima[3*id1+0]<<"miny"<<minima[3*id1+1]<<"minz"<<minima[3*id1+2]<<std::endl;
		std::cout<<id2<<"minx"<<minima[3*id2+0]<<"miny"<<minima[3*id2+1]<<"minz"<<minima[3*id2+2]<<std::endl;
		std::cout<<id1<<"maxx"<<maxima[3*id1+0]<<"maxy"<<maxima[3*id1+1]<<"maxz"<<maxima[3*id1+2]<<std::endl;
		std::cout<<id2<<"maxx"<<maxima[3*id2+0]<<"maxy"<<maxima[3*id2+1]<<"maxz"<<maxima[3*id2+2]<<std::endl;
		//**********
		*/
}
		...
		for (size_t i = 0; i < nBounds; i++) {
			...
			if (b) {
				...
				BBji.coord = ((BBji.flags.hasBB = ((bool)bv)) ? (BBji.flags.isMin ? bv->min[j] : bv->max[j])
				        ...
				        //std::cout<<"(bool)bv="<<(bool)bv<<std::endl;//=1
...
#else
				std::cout<<"YADE_REAL_BIT>64"<<std::endl;
				if (bv) {
...
/* Performance hint
	================
	.saize()<<"minima size:"<<minima.size<<std::endl;
...
boost::python::tuple InsertionSortCollider::dumpBounds()
{
	...
	for (int axis = 0; axis < 3; axis++) {
		...
		if (periodic) {
			...
			for (size_t i = 0; i < V.size(); i++) {
				bl[axis].append(boost::python::make_tuple(V[i].coord, (V[i].flags.isMin ? -1 : 1) * V[i].id, V[i].flags.hasBB, i, V.size()));
			}
		}
	}
...
//**************** 输出bounds序列至文件中 2024-3-18 
void InsertionSortCollider::outputBounds(const char* filename, int choose) {
    std::cout << "writing bounds into a file" << std::endl;
    std::ofstream f;
    f.open(filename);
    ...
    for (int axis = 0; axis < 3; axis++) {
		...
		if (!periodic) {
			for (size_t i = 0; i < V.size(); i++) {
				f<<V[i].coord<<"  "<<((V[i].flags.isMin ? -1 : 1) * V[i].id)<<"  "<<V[i].flags.hasBB<<"  "<<i<<"  "<<V.size()<<endl;
			}
		}
	        f<<"bound line-------------------------------------"<<std::endl;
     }
     }
     else{
     std::cout<<"minima size"<<minima.size()<<"maxima size"<<maxima.size()<<std::endl;
     for (size_t i = 0; i < maxima.size(); i++) {
			f<<"maxima i:"<<i<<"  "<<maxima[i]<<std::endl;
		}
		f<<"bound line-------------------------------------"<<std::endl;
		for (size_t i = 0; i < minima.size(); i++) {
			f<<"minima i:"<<i<<"  "<<minima[i]<<std::endl;
		}
		f<<"bound line-------------------------------------"<<std::endl;
     }
     f.close();
