...
Vector3r getSurface(Real R, Vector2r phi) {
	assert(phi[0] >= 0);
	double x, y, z;
	x = R * cos(phi(1))*cos(phi(0));
	y = R * cos(phi(1))*sin(phi(0));
	z = R * sin(phi(1));
	return Vector3r(x, y, z);
}

void outputVTK(const char* filename, int slices) {
	std::cout << "writing sphere into a VTK file" << std::endl;
	std::ofstream f;
	f.open(filename);
	...
	for (; bi != biEnd; ++bi)
	{
		...
		if (b->shape->getClassName() == "Sphere") {
			point_counter++;
			const shared_ptr<Sphere>& A = YADE_PTR_DYN_CAST<Sphere>(b->shape);
			radius = A->radius;
			...
			for (a = 0.0, i = 0; i < h; i++, a += hStep)
				for (b1 = 0.0, j = 0; j < w; j++, b1 += wStep)
				{
					Vector3r Surf = getSurface(radius,Vector2r(b1, a - M_PI_2l));//b经度线  a-纬度线
	...
	f.close();
	std::cout << "writing sphere into a VTK file succusful" << std::endl;
