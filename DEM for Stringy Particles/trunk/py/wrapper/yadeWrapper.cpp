...
//************************4-10
class pyNodeNeighborContainer {//py膜节点容器类
//private:
	//typedef std::map<Node::id_t, Se3r> MemberMap;
public:
	const shared_ptr<NodeNeighborContainer> proxee;
	//pyNodeIterator pyIter() { return pyNodeIterator(proxee); }
	pyNodeNeighborContainer(const shared_ptr<NodeNeighborContainer>& _proxee) : proxee(_proxee) {}//构造函数
	//shared_ptr<Node> pyGetitem(Body::id_t _id) {
	...
	//}
	/*std::vector<Body::id_t>*/py::tuple append([[maybe_unused]] int _cows,[[maybe_unused]] int _clos,const py::list & pylist/*std::vector<std::vector<Body::id_t>> Matrix*/) {
		std::vector<std::vector<Body::id_t>> cppMatrix;
		int outerLength = boost::python::len(pylist);
	        for (int i = 0; i < outerLength; ++i) {
        		py::list innerPyList = py::extract<boost::python::list>(pylist[i]);
       		std::vector<Body::id_t> innerVector;
        		int innerLength = py::len(innerPyList);
       	        for (int j = 0; j < innerLength; ++j) {
          			  Body::id_t value = py::extract<Body::id_t>(innerPyList[j]);
            			innerVector.push_back(value);
       		 }

        			cppMatrix.push_back(innerVector);
        			innerVector.clear();
   		 }
   		 
		std::vector<std::vector<Body::id_t>>::iterator first = cppMatrix.begin();
		/*std::vector<std::vector<Body::id_t>>::iterator last = Matrix.end();
		std::vector<std::vector<Body::id_t>>::iterator Real = std::prev(last);*/
		int Ncows = cppMatrix.size();
		int Ncols = (*first).size();
		assert((_cows == Ncows) && (_cols == Ncols));
		std::vector<Body::id_t> ret;
		ret= proxee->insert(Ncows, Ncols, cppMatrix);
		std::vector<Body::id_t>::iterator idfirst = ret.begin();
		std::vector<Body::id_t>::iterator idend = ret.end();
		std::vector<Body::id_t> top(idfirst, idfirst +Ncols);
		std::vector<Body::id_t> free(idfirst + Ncols, idend - Ncols);
		std::vector<Body::id_t> bottom(idend- Ncols, idend);
		return py::make_tuple(top, free, bottom);
		//return ret;
	}
	
	shared_ptr<NodeNeighbor> pyGetitem(Body::id_t _id)
	{
		auto it = std::find(proxee->Self.begin(), proxee->Self.end(), _id);
		
		//if()
		//int id = (_id >= 0 ? _id : proxee->size() + _id);
		if (_id < 0 || it == proxee->Self.end()) {
			PyErr_SetString(PyExc_IndexError, "Body id out of range or no exit.");
			py::throw_error_already_set(); /* make compiler happy; never reached */
			return shared_ptr<NodeNeighbor>();
		}
		else {
			int index = std::distance(proxee->Self.begin(), it);//距离
			return	proxee->Neighbor[index];
		}
	}

	//vector<Node::id_t> replace(vector<shared_ptr<NodeNeighbor> > nn) { proxee->clear(); return appendList(bb); }
	long length() { return proxee->size(); }
	void clear() { proxee->clear(); }
	//bool erase(Node::id_t id) { return proxee->erase(id); }
};
//************************4-10
class pyThreadContainer {//py膜节点容器类

public:
	const shared_ptr<ThreadContainer> proxee;
	pyThreadContainer(const shared_ptr<ThreadContainer>& _proxee) : proxee(_proxee) {}//构造函数

	int append(shared_ptr<Thread> t)
	{
		return proxee->insert(t);
	}
	
	vector<int> appendList(vector<shared_ptr<Thread>> tt)
	{
		vector<int>                ret;
		for (const auto& t : tt) {
			ret.push_back(append(t));
		}
		return ret;
	}
	...
	shared_ptr<Thread> pyGetitem( int _id)
	{
		int id = (_id >= 0 ? _id : proxee->size() + _id);
		if (id < 0 || id >= proxee->size()) {
...
class pyNodeForceContainer{
	...
	public:
		...
		void checkId(long id){ if(id<0 || (size_t)id>=scene->_nodes->size()){ PyErr_SetString(PyExc_IndexError, "Node id out of range."); py::throw_error_already_set(); /* never reached */ throw; } }
...
//*************************************
pyNodeNeighborContainer nodes_get(void) { assertScene(); /*cout<<"nodes_get from scene"<<endl;*/return pyNodeNeighborContainer(OMEGA.getScene()->nodes); }
pyThreadContainer threads_get(void){assertScene();return pyThreadContainer(OMEGA.getScene()->threads);}//6-5
//*************************************
//*****
pyNodeContainer _nodes_get(void){assertScene(); return pyNodeContainer(OMEGA.getScene()->_nodes); }
//*******
pyBodyContainer bodies_get(void)
{
	assertScene();
	//cout<<"bodies_get from scene"<<endl;
...
try {
	...
	py::class_<pyOmega>("Omega")
	        ...
	        .add_property(
	         ...
	         .add_property("_nodes",&pyOmega::_nodes_get,"Nodes in the current simulation (container supporting index access by id and iteration)")//******************
	         .add_property("threads",&pyOmega::threads_get,"Nodes in the current simulation (container supporting index access by id and iteration)")//******************
	...
	py::class_<pyTags>(
       ...
       py::class_<pyNodeNeighborContainer>("NodeNeighborContainer", py::init<pyNodeNeighborContainer&>())//************************
       	.def("__getitem__", &pyNodeNeighborContainer::pyGetitem)
		.def("__len__", &pyNodeNeighborContainer::length)
		.def("append", &pyNodeNeighborContainer::append, "Append one Body instance, return its id.")
		.def("clear", &pyNodeNeighborContainer::clear, "Remove all bodies (interactions not checked)");
	py::class_<pyThreadContainer>("pyThreadContainer", py::init<pyThreadContainer&>())//************************6-5
		 .def("__getitem__", &pyThreadContainer::pyGetitem)
		.def("__len__", &pyThreadContainer::length)
		.def("append", &pyThreadContainer::append, "Append one Body instance, return its id.")
		.def("append", &pyThreadContainer::appendList, "Append one Body instance, return its id.")
		.def("clear", &pyThreadContainer::clear, "Remove all bodies (interactions not checked)");
