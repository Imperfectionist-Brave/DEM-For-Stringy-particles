...
try {
	...
	boost::python::class_<SpherePack>(
		...
		.def("makeNode",
	             &SpherePack::makeNode,
	             (boost::python::arg("CCenter")      = Vector3r(Vector3r::Zero()),
	              boost::python::arg("CRadius")      = 0,
	              boost::python::arg("CHeight")        = 0,
	             // boost::python::arg("start")          = Vector3r(Vector3r::Zero()),
	              boost::python::arg("nodeRadius")     = -1,
	              boost::python::arg("gap")				= true,
	              boost::python::arg("up")       = false),
	             R"""(Create a random cloud of particles enclosed in a parallelepiped. The resulting packing is a gas-like state with no contacts between particles initially. Usually used as a first step before reaching a dense packing.

				)""")
		.def("getArguments", &SpherePack::getArguments,"get num cows and cols return list")
		    //***********
	        .def("psd",
	             &SpherePack::psd,
