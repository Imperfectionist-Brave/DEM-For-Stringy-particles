...
{
	isPeriodic = periodic;
	if(pack.size()>0)pack.clear();//*********************************4-15
...
//************************
long SpherePack::makeNode(
	Vector3r            cylinderCenter,
	Real                cylinderRadius,
	Real                cylinderHeight,
	//Vector3r            start,//生成点
	Real                nodeRadius,
	...
	bool                up) 
{
	if(pack.size()>0)pack.clear();
	//Vector3r startpoint1 = cylinderCenter + start * (cylinderRadius + nodeRadius);
	int cows = 0;
	int cols = 0;
	Real gapratio=0.8;
	//Real dx = 0;
	Real dz = 0;
	Real un = 0;
	Real Rtotal = gapratio*nodeRadius + cylinderRadius;
	Real ratio = nodeRadius / (Rtotal);
	cols = int(Mathr::PI/ asin(ratio));
	cols = (gap? cols : cols + 1);//圆周数量 列数量
	Real theta = 2*Mathr::PI/ cols;
	un = 2 * (sin(0.5*theta)*Rtotal - nodeRadius);
	assert(un >= 0 && gap);
	dz = sqrt(3)*(0.5*un + nodeRadius);
	cows = int(cylinderHeight / dz) + 1;//行数量
	Real realHei=(cows-1)*dz;
	Real deta=0.5*(realHei-cylinderHeight);
	//axis = Vector3r(0, 0, 1);
	//std::vector<std::vector<Vector3r>> pos2;
	//pos2.resize(cows);
	Vector3r c = Vector3r::Zero();//坐标
	Real initial = 0.5*theta;//角度偏移量
	for (int i = 0; i < cows;i++) {
		for (int j = 0; j < cols; j++) {
			//Quaternion q= Quaternionr(AngleAxisr(i*theta, axis));
			/*std::vector<Vector3r> pos1;
			pos1.resize(cols);*/
			if (!up) {
					c[0] = Rtotal * cos((i % 2 == 0 ? 0 : initial) + j*theta);//从1，0，0生成
					c[1] = Rtotal * sin((i % 2 == 0 ? 0 : initial) + j*theta);
					c[2] = deta-i * dz;
			}
			else{
					c[0] = Rtotal * cos((i % 2 == 0 ? 0 : initial) + j * theta);
					c[1] = Rtotal * sin((i % 2 == 0 ? 0 : initial) + j * theta);
					c[2] = deta+i * dz;
			}
			c = c + cylinderCenter;
...
py::list SpherePack::getArguments() const
{
	py::list pyList;
