...
#ifdef YADE_OPENMP
	//cout<<"omp_max="<<omp_get_max_threads()<<endl;
#pragma omp parallel for schedule(guided) num_threads(ompThreads > 0 ? min(ompThreads, omp_get_max_threads()) : omp_get_max_threads())
	...
	for (long i = 0; i < size; i++) {
		//clock_t start = clock();
		//cout<<"num threads"<<omp_get_thread_num()<<"of"<<omp_get_num_threads()<<endl;
		...
		bool geomCreated;
		if (!scene->isPeriodic) {
			geomCreated = I->functorCache.geom->go(b1->shape, b2->shape, *b1->state, *b2->state, Vector3r::Zero(), /*force*/ false, I);//创建geom
			//std::cout<<I->getId1()<<"and"<<I->getId2()<<"geomcreated="<<geomCreated<<endl;//********************************************************测试代码
		} else {
			// handle periodicity
		...
		//clock_t end = clock();
		//cout <<"循环次数"<<i<<"运行时间"<<double(end - start) / CLOCKS_PER_SEC << "s" << endl;
	}
}
