...
shared_ptr<Interaction> IGeomDispatcher::explicitAction(const shared_ptr<Body>& b1, const shared_ptr<Body>& b2, bool force)
{
	...
	//添加测试代码
	//std::cout<<"shift2 in dispatching ="<<shift2(0)<<" "<<shift2(1)<<" "<<shift2(2)<<std::endl;
	updateScenePtr();
...
void IPhysDispatcher::explicitAction(shared_ptr<Material>& pp1, shared_ptr<Material>& pp2, shared_ptr<Interaction>& I)
{
	updateScenePtr();
	//添加测试代码
	//std::cout<<"IPhysDispatcher::explicitAction"<<std::endl;
...
void IPhysDispatcher::action()
{
	updateScenePtr();
	//添加测试代码
	std::cout<<"IPhysDispatcher::action"<<std::endl;
	shared_ptr<BodyContainer>& bodies = scene->bodies;
#ifdef YADE_OPENMP
...
void LawDispatcher::action()
{
	updateScenePtr();
	//添加测试代码
	std::cout<<"LawDispatcher::action"<<std::endl;
#ifdef YADE_OPENMP
	const long size = scene->interactions->size();
#pragma omp parallel for
	for (long i = 0; i < size; i++) {
